...
public class RectTool extends Constants{
	public Main cls;
	...
	public RectTool(Main cls,ImageEditorWindow i){
		this.cls=cls;
		this.i=i;
	}
	...
	public void update(){
		if (this.cls.MOUSE==1&&this.cls.MOUSE_COUNT==1&&this.cls.MOUSE_BUTTON==1){
			...
			g.fillRect(Math.min(this.s.x,this.e.x),Math.min(this.s.y,this.e.y),Math.abs(this.e.x-this.s.x)+1,Math.abs(this.e.y-this.s.y)+1);
			g.dispose();
			this.s=null;
			this.e=null;
		}
